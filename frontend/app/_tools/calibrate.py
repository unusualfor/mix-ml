"""In-container calibration of 2D flavor map clusters.

Lives at `app._tools.calibrate` so it can be invoked from the running
frontend container against the local backend on the docker network:

    docker exec --user 0 mix-ml-frontend \
        python -m app._tools.calibrate --backend-url http://backend:8080 --write

The script reuses the same UMAP + DBSCAN parameters and the same
`_build_feature_matrix` as the runtime builder, so its DBSCAN partition
always matches what the running app shows on `/inventory/flavor-map`.

Preserves existing impressions where the bottle set of a new cluster
overlaps an old one by ≥75%.
"""
...
IMPRESSIONS_FILE = Path("/opt/app-root/src/app/data/cluster_impressions.json")


def fetch_bottles(backend_url: str) -> list[dict]:
    resp = httpx.get(
        f"{backend_url}/api/bottles", params={"limit": 200}, timeout=30
    )
...
def run_dbscan(embedding: np.ndarray, eps: float, min_samples: int) -> np.ndarray:
    return DBSCAN(eps=eps, min_samples=min_samples).fit_predict(embedding)
...
def match_existing_impression(cluster_bottles: frozenset, existing: dict) -> str | None:
    if not existing:
        return None
    for _cid, comp in existing.get("clusters", {}).items():
        known = frozenset((b[0], b[1]) for b in comp.get("bottles", []))
...
def match_existing_outlier(brand: str, label: str | None, existing: dict) -> str | None:
    if not existing:
        return None
...
def build_output(
    ...
    output = {
        "_meta": {
            ...
            "generated_with": "app._tools.calibrate",
            "umap_random_state": 42,
            "dbscan_eps": eps,
            "dbscan_min_samples": min_samples,
            "note": (
                "Regenerate after adding/removing bottles. Auto-generation kicks "
                "in for unmatched clusters."
            ),
        },
        "clusters": {},
        "outliers": {},
...
def main() -> None:
    ...
    parser.add_argument("--backend-url", default="http://backend:8080")
    ...
    parser.add_argument(
        "--output",
        default=None,
        help=f"Override output path (default: {IMPRESSIONS_FILE})",
    )
    args = parser.parse_args()

    out_path = Path(args.output) if args.output else IMPRESSIONS_FILE
    ...
    print(
        f"Running DBSCAN (eps={args.eps}, min_samples={args.min_samples})...",
        file=sys.stderr,
    )
    ...
    if args.write:
        ...
        print(f"\n(Use --write to save to {out_path})", file=sys.stderr)
